...
def select_diverse_samples(embeddings, texts, k=3):
    """Select the k most different samples"""
    if len(embeddings) <= k:
    ...
    while len(selected) < k:
        ...
        # Select the sample that is least similar to the selected sample
...
def calculate_rouge(reference, hypothesis):
    """Calculate ROUGE score for a single sentence"""
    scorer = rouge_scorer.RougeScorer(['rouge1'], use_stemmer=True)
    scores = scorer.score(reference, hypothesis)
    return scores['rouge1'].fmeasure

def calculate_comet_score(model, source, reference, hypothesis):
    """Calculate COMET score for a single sentence"""
...
def calculate_batch_comet_score(comet_model, sources, references, hypotheses):
    """Calculate COMET score for a batch of sentences"""
    data = [
...
def get_translation_prompt(german_text):
    prompt = f"""Translate the following German text to English. Provide only the English translation without any additional explanation.

German: {german_text}
English:"""
    return prompt

def pos_translation_prompt_v1(german_text, previous_list):
    # Organize previous translations into a list format
    prompt = f"""Translate the following German text to English. Generate a new translation that is significantly different from the previous translations while maintaining accuracy and fluency.
...
def neg_translation_prompt(german_text, previous_list):
    # Organize previous translations into a list format
    
    prompt = f"""Translate the following German text to English. Generate a new translation that is very similar to the previous translations, maintaining the same word choices and sentence structure whenever possible.
...
- Keep the sentence structure highly similar
    ...
    for j in range(1):
        for i in range(args.iter_num):
            ...
            for num_example, example in enumerate(test_data):
                ...
                prompt = get_translation_prompt(example['translation']['de'])
                prompt = get_templated_prompt(prompt, tokenizer)
                if i!=0:
                    for index in range(1):
                        pos_prompt = pos_translation_prompt_v1(example['translation']['de'],prefix_outputs[num_example])
                        neg_prompt = neg_translation_prompt(example['translation']['de'],prefix_outputs[num_example])
            ...
            for idx, output in enumerate(tqdm(outputs, desc=f"Compressing outputs (iteration {i})")):
                ...
                with torch.no_grad():
                    ...
                    if num_generated_tokens > 0:  
                        ...
                        # Get the hidden states for each generation step (for the generated token)
            ...
            sources = [example['translation']['de'] for example in test_data]
            references = [example['translation']['en'] for example in test_data]

            for example, output in zip(test_data, outputs):
                sentence_bleu_score = calculate_sentence_bleu(example['translation']['en'], output)
                individual_bleu_scores.append(sentence_bleu_score)

                sentence_rouge_score = calculate_rouge(example['translation']['en'], output)
                individual_rouge_scores.append(sentence_rouge_score)

                predictions.append({
                    "source_de": example['translation']['de'],
            ...
            all_predictions[i] = {
                ...
                "AVG_ROUGE1": np.mean(individual_rouge_scores),
